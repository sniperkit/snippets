...
var tpl = template.Must(template.New("templates").Parse(`package templates
...
const (
	TemplatesBuildDate = "{{.BuildDate}}"
)

func Templates() map[string][]byte {
	var assets = make(map[string][]byte, {{.Templates | len}})
{{range $asset := .Templates}}
...
func walkerFor(basePath string) filepath.WalkFunc {
	return func(name string, info os.FileInfo, err error) error {
		...
		if info.Mode().IsRegular() {
			...
			templates = append(templates, tmplItem{
